const RVPdfDeifinition = (data, create) => {
    ...
    const body = products.map((product) => {
        return [
            product.RV_Product.no,
            product.name,
            product.RV_Product.amount,
            { text: product.RV_Product.price.toLocaleString(), alignment: 'right' },
        ];
    });

    const total_amount = products.reduce((prev, curr) => {
        return prev + curr.RV_Product.amount;
    }, 0);
...
module.exports = { RVPdfDeifinition };
